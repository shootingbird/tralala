...
export const AuthModal = ({ isOpen, onClose }: AuthModalProps) => {
    const { login, signup, verifyEmail, resendVerificationCode, isAuthenticated, user } = useAuth();
            ...
            if (result.success) {
                ...
                setTimeout(async () => {
                    ...
                    if (loginResult.success) {
                        ...
                        setErrorMessage(loginResult.error || 'Auto-login failed after verification');
                        setShowErrorModal(true);
                    }
                }, 1500);
            } else {
    ...
    return (
        <div className={`fixed inset-0 z-[100] ${isOpen ? 'block' : 'hidden'}`}>
            <div className="fixed inset-0 bg-black/50 touch-none z-[101]" onClick={() => { onClose(false); resetModal(); }} />
            <div className="fixed inset-x-0 bottom-0 top-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 w-full md:w-[480px] bg-white md:rounded-2xl md:max-h-[80vh] h-full md:h-auto overflow-y-auto z-[102]">
                <div className="p-6 md:p-8">
                    <button
                        onClick={() => { onClose(false); resetModal(); }}
                    ...
                    {!showOtpVerification ? (
                        ...
                        <>
                            <div className="text-center mb-8">
                                <h2 className="text-base md:text-lg font-semibold mb-2">Verify Your Email</h2>
                ...
                <Modal
                    ...
                    type="success"
                    title={showOtpVerification && otpCode.length === 6 ? "Correct OTP!" : (showOtpVerification ? "Code Sent!" : (isLogin ? "Authentication Successful" : "Account Created!"))}
                    message={showOtpVerification && otpCode.length === 6 
